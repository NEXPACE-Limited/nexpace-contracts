...
import { CreatorWallet, CreatorWalletV2, DAppRewardAllocationWallet } from "../../typechain-types";
...
interface CreatorWalletInfo {
  ...
  threshold: number;
  account?: CreatorWallet | CreatorWalletV2;
  nesoAmount: number;
...
describe("Creator Contract", () => {
  ...
  async function fixture() {
    ...
    const CreatorImpl2 = await ethers.getContractFactory("CreatorWalletV2");
    ...
    const CreatorWalletV2 = await ethers.getContractFactory("CreatorWalletV2");
    ...
    const creatorImpl2 = await CreatorImpl2.deploy(await forwarder.getAddress(), neso.address, creatorFactory.address);
  ...
  describe("Transfer ERC20/721/1155", () => {
    it("HappyPath", async () => {
      const { alice, eventHelper, creatorWalletInfo, dAppInfo, neso, token721, token1155, beacon, creatorImpl2 } =
        await loadFixture(fixture);
      ...
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();

      await neso.connect(alice).deposit({ value: "1" });
      await neso.connect(alice).transfer(creatorWallet.address, 100);
      await token721.safeMint(creatorWallet.address, 102);
      await token721.safeMint(creatorWallet.address, 103);
      await token1155.mintBatch(creatorWallet.address, [1, 2, 3], [10, 20, 30], "0x");
      await creatorWalletCall(
        eventHelper,
        creatorWalletInfo[1],
      ...
      await creatorWalletCall(
        eventHelper,
        creatorWalletInfo[1],
        creatorWallet.interface.encodeFunctionData("transferERC721", [token721.address, dApp.address, 102])
      );
      await creatorWalletCall(
        eventHelper,
        creatorWalletInfo[1],
        creatorWallet.interface.encodeFunctionData("transferERC721", [token721.address, dApp.address, 103])
      );
      await creatorWalletCall(
        eventHelper,
        creatorWalletInfo[1],
  ...
  describe("CreatorWallet - requestItemIssuance", () => {
    it("Success", async () => {
      ...
      const txnData = (creatorWallet as CreatorWallet)!.interface.encodeFunctionData(
        "requestItemIssuance",
        [1, 100, 1]
      );
    ...
    it("Revert - when called non-self", async () => {
      const { creatorWalletInfo } = await loadFixture(fixture);
      const { account: creatorWallet } = creatorWalletInfo[0];
      await expect((creatorWallet as CreatorWallet)!.requestItemIssuance(1, 100, 1)).to.be.revertedWith(
        nxErrors.SelfCallUpgradeable.forbidden
      );
    });
  });
  ...
  // CreatorWalletV2
  describe("CreatorWalletV2 - constructor", function () {
    it("CreatorWalletV2: should be reverted when deploy with zero address", async function () {
      const CreatorWallet = await ethers.getContractFactory("CreatorWalletV2");
      await expect(
        CreatorWallet.deploy(ethers.constants.AddressZero, ethers.constants.AddressZero, ethers.constants.AddressZero)
      ).to.be.revertedWith(nxErrors.CreatorWalletLogicUpgradeable.validAddress);
    });
  ...
  describe("CreatorWalletV2 - Transfer ERC20/721/1155", () => {
    it("CreatorWalletV2: HappyPath", async () => {
      const { alice, eventHelper, creatorWalletInfo, dAppInfo, neso, token721, token1155, beacon, creatorImpl2 } =
        await loadFixture(fixture);
      const creatorWallet = creatorWalletInfo[1].account!;
      const dApp = dAppInfo[0].account!;
  ...
  describe("CreatorWalletV2 - initialize", () => {
    it("CreatorWalletV2: Success", async () => {
      const { alice, bob, creatorFactory, factoryExecutor, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const creatorWalletInfo: CreatorWalletInfo = {
        name: "CreatorV2",
        owners: [alice, bob],
      ...
      await creatorFactory.connect(factoryExecutor).addCreator(
        creatorWalletInfo.name,
        creatorWalletInfo.owners.map((v) => v.getAddress()),
        creatorWalletInfo.threshold
      );
    });

    it("CreatorWalletV2: Revert - when called as a duplicate", async () => {
      const { creatorWalletInfo, alice, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const creatorWallet = creatorWalletInfo[0].account!;
      await expect(creatorWallet.initialize([alice.getAddress()], 3)).to.be.revertedWith(
        nxErrors.Initializable.alreadyInitialized
  ...
  describe("CreatorWalletV2 - onlyInitializing", () => {
    it("CreatorWalletV2: Revert - when not called initializing", async () => {
      const { creatorFactory, neso2 } = await loadFixture(fixture);
      const MockCreatorWalletLogic = await ethers.getContractFactory("MockCreatorWalletLogicV2");
      const mockCreatorWalletLogic = await MockCreatorWalletLogic.deploy(neso2.address, creatorFactory.address);
      await mockCreatorWalletLogic.initialize();
      await expect(mockCreatorWalletLogic.f()).to.be.revertedWith(nxErrors.Initializable.notInitializing);
      await expect(mockCreatorWalletLogic.g()).to.be.revertedWith(nxErrors.Initializable.notInitializing);
  ...
  describe("CreatorWalletV2 - allocateERC20(address,uint32,uint256)", () => {
    it("CreatorWalletV2: Success(NESO)", async () => {
      const { neso, eventHelper, creatorWalletInfo, dAppInfo, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const creatorInfo = creatorWalletInfo[1];
      const creatorAccount = creatorWalletInfo[1].account!;
      const txnData = creatorAccount.interface.encodeFunctionData("allocateERC20(address,uint32,uint256)", [
    ...
    it("CreatorWalletV2: Success(NESO) - all token", async () => {
      const { neso, eventHelper, creatorWalletInfo, dAppInfo, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const creatorInfo = creatorWalletInfo[1];
      const creatorAccount = creatorWalletInfo[1].account!;
    ...
    it("CreatorWalletV2: Revert - when called Set 'to' to non-self", async () => {
      const { creatorWalletInfo, neso, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData = creatorWalletInfo[0].account!.interface.encodeFunctionData(
        "allocateERC20(address,uint32,uint256)",
        [neso.address, 2, 300]
    ...
    it("CreatorWalletV2: Revert - when called non-self", async () => {
      const { creatorWalletInfo, neso, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      await expect(
        creatorWalletInfo[0].account!["allocateERC20(address,uint32,uint256)"](neso.address, 1, 300)
    ...
    it("CreatorWalletV2: Revert - when called invalid dAppId", async () => {
      const { eventHelper, creatorWalletInfo, neso, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData = creatorWalletInfo[1].account!.interface.encodeFunctionData(
        "allocateERC20(address,uint32,uint256)",
        [neso.address, 2, 300]
    ...
    it("CreatorWalletV2: Revert - when called inactive dAppId", async () => {
      const { neso, eventHelper, creatorFactory, creatorWalletInfo, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const creatorInfo = creatorWalletInfo[1];
      const creatorAccount = creatorWalletInfo[1].account!;
      await creatorFactory.setDAppActivation(1, false);
  ...
  describe("CreatorWalletV2 - allocateERC20(address,address,uint256)", () => {
    it("CreatorWalletV2: Success(NESO)", async () => {
      const { neso, eventHelper, creatorWalletInfo, dAppInfo, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const creatorInfo = creatorWalletInfo[1];
    ...
    it("CreatorWalletV2: Revert - when called Set 'to' to non-self", async () => {
      const { creatorWalletInfo, neso, dAppInfo, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData = creatorWalletInfo[0].account!.interface.encodeFunctionData(
        "allocateERC20(address,address,uint256)",
        [neso.address, dAppInfo[1].account!.address, 300]
    ...
    it("CreatorWalletV2: Revert - when called invalid dAppId", async () => {
      const { eventHelper, creatorWalletInfo, neso, dAppInfo, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData = creatorWalletInfo[1].account!.interface.encodeFunctionData(
        "allocateERC20(address,address,uint256)",
        [neso.address, dAppInfo[1].account!.address, 300]
  ...
  describe("CreatorWalletV2 - allocateItems", () => {
    it("CreatorWalletV2: Success", async () => {
      const { eventHelper, CreatorWallet, creatorWalletInfo, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData = CreatorWallet.interface.encodeFunctionData("allocateItems", [1, "0x0123456789abcdef"]);
      await expect(creatorWalletCall(eventHelper, creatorWalletInfo[1], txnData))
        .to.be.emit(creatorWalletInfo[1].account!, "ItemsAllocated")
    ...
    it("CreatorWalletV2: Revert - when called non-self", async () => {
      const { creatorWalletInfo, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      await expect(creatorWalletInfo[0].account!.allocateItems(1, "0x0123456789abcdef")).to.be.revertedWith(
        nxErrors.SelfCallUpgradeable.forbidden
      );
  ...
  describe("CreatorWalletV2 - withdraw NXPC", () => {
    it("CreatorWalletV2: Success", async () => {
      const { eventHelper, creatorWalletInfo, alice, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData = creatorWalletInfo[1].account!.interface.encodeFunctionData("transferNXPC", [
        await alice.getAddress(),
        10,
    ...
    it("CreatorWalletV2: Revert - when called not enough nxpc", async () => {
      const { eventHelper, creatorWalletInfo, alice, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData = creatorWalletInfo[1].account!.interface.encodeFunctionData("transferNXPC", [
        await alice.getAddress(),
  ...
  describe("CreatorWalletV2 - withdraw NESO", () => {
    it("CreatorWalletV2: Success", async () => {
      const { eventHelper, creatorWalletInfo, alice, neso, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData = creatorWalletInfo[1].account!.interface.encodeFunctionData("withdrawERC20", [
        neso.address,
        await alice.getAddress(),
    ...
    it("CreatorWalletV2: Success - all", async () => {
      const { eventHelper, creatorWalletInfo, alice, neso, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData = creatorWalletInfo[1].account!.interface.encodeFunctionData("withdrawERC20", [
        neso.address,
    ...
    it("CreatorWalletV2: Revert - when called non-self", async () => {
      const { creatorWalletInfo, alice, neso, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      await expect(
        creatorWalletInfo[0].account!.withdrawERC20(neso.address, alice.getAddress(), 10)
      ).to.be.revertedWith(nxErrors.SelfCallUpgradeable.forbidden);
  ...
  describe("CreatorWalletV2 - exchange NESO to NXPC", () => {
    it("CreatorWalletV2: Success", async () => {
      const { eventHelper, creatorWalletInfo, neso, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData1 = creatorWalletInfo[1].account!.interface.encodeFunctionData("transferNXPC", [neso.address, 1]);
  ...
  describe("CreatorWalletV2 - batchTransferERC1155(address,address,uint256[],uint256[])", () => {
    it("CreatorWalletV2: Success", async () => {
      const { eventHelper, creatorWalletInfo, alice, token1155, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const { account: creatorWallet } = creatorWalletInfo[1];
      await token1155.mintBatch(creatorWallet!.address, [10, 20], [5, 10], "0x");
      const txnData = creatorWallet!.interface.encodeFunctionData("batchTransferERC1155", [
    ...
    it("CreatorWalletV2: Revert - when called non-self", async () => {
      const { creatorWalletInfo, alice, token1155, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const { account: creatorWallet } = creatorWalletInfo[1];
      await token1155.mintBatch(creatorWallet!.address, [10, 20], [5, 10], "0x");
      await expect(
  ...
  describe("CreatorWalletV2 - transferERC721", () => {
    it("CreatorWalletV2: Success", async () => {
      const { eventHelper, CreatorWallet, creatorWalletInfo, alice, token721, beacon, creatorImpl2 } =
        await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      await token721.safeMint(creatorWalletInfo[1].account!.address, 10);
      const txnData = CreatorWallet.interface.encodeFunctionData("transferERC721", [
    ...
    it("CreatorWalletV2: Revert - when called non-self", async () => {
      const { creatorWalletInfo, alice, token721, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      await token721.safeMint(creatorWalletInfo[1].account!.address, 10);
      await expect(
        creatorWalletInfo[0].account!.transferERC721(token721.address, await alice.getAddress(), 10)
  ...
  describe("CreatorWalletV2 - batchTransferERC721", () => {
    it("CreatorWalletV2: Success", async () => {
      const { eventHelper, CreatorWallet, creatorWalletInfo, alice, token721, beacon, creatorImpl2 } =
        await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
    ...
    it("CreatorWalletV2: Revert - when called invalid length", async () => {
      const { eventHelper, CreatorWallet, creatorWalletInfo, alice, token721, beacon, creatorImpl2 } =
        await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      await token721.safeMint(creatorWalletInfo[1].account!.address, 10);
  ...
  describe("CreatorWalletV2 - transferERC1155", () => {
    it("CreatorWalletV2: Success", async () => {
      const { eventHelper, creatorWalletInfo, alice, token1155, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const { account: creatorWallet } = creatorWalletInfo[1];
  ...
  describe("CreatorWalletV2 - requestItemIssuance(uint24,uint96,uint256)", () => {
    it("CreatorWalletV2: Revert - deprecated", async () => {
      const { eventHelper, creatorWalletInfo, beacon, creatorImpl2, CreatorWalletV2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const txnData = CreatorWalletV2!.interface.encodeFunctionData(
        "requestItemIssuance(uint24,uint96,uint256)",
        [1, 100, 1]
      );
      await expect(creatorWalletCall(eventHelper, creatorWalletInfo[0], txnData)).to.be.revertedWith(
        nxErrors.CreatorWalletLogicUpgradeable.deprecated
      );
    });
    it("CreatorWalletV2: Revert - when called non-self", async () => {
      const { creatorWalletInfo, beacon, creatorImpl2, CreatorWalletV2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const { account } = creatorWalletInfo[0];
      const creatorWallet = CreatorWalletV2.attach(account!.address);
      await expect(creatorWallet!["requestItemIssuance(uint24,uint96,uint256)"](1, 100, 1)).to.be.revertedWith(
        nxErrors.SelfCallUpgradeable.forbidden
      );
    });
  });

  describe("CreatorWalletV2 - requestItemIssuance(uint24,uint96,uint256,uint256)", () => {
    it("CreatorWalletV2: Success", async () => {
      const { eventHelper, creatorWalletInfo, itemIssuance, beacon, creatorImpl2, CreatorWalletV2 } = await loadFixture(
        fixture
      );
      const { account: creatorWallet } = creatorWalletInfo[0];
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const requireNXPC = 100;
      const txnData = CreatorWalletV2!.interface.encodeFunctionData(
        "requestItemIssuance(uint24,uint96,uint256,uint256)",
        [1, 100, 1, requireNXPC]
      );
      await expect(creatorWalletCall(eventHelper, creatorWalletInfo[0], txnData))
        .to.emit(itemIssuance, "ItemRequested")
        .withArgs(1, 1, creatorWallet!.address, 100, 1, requireNXPC);
    });
    it("CreatorWalletV2: Revert - when called non-self", async () => {
      const { creatorWalletInfo, beacon, creatorImpl2, CreatorWalletV2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      ...
      const creatorWallet = CreatorWalletV2.attach(account!.address);
      await expect(
        creatorWallet["requestItemIssuance(uint24,uint96,uint256,uint256)"](1, 100, 1, 100)
      ).to.be.revertedWith(nxErrors.SelfCallUpgradeable.forbidden);
    });
  });

  describe("CreatorWalletV2 - meta transaction", () => {
    it("CreatorWalletV2: Transfer ERC721", async () => {
      const { creatorWalletInfo, alice, token721, forwarder, beacon, creatorImpl2 } = await loadFixture(fixture);
      await (await beacon.upgradeTo(creatorImpl2.address)).wait();
      const { account: creatorWallet } = creatorWalletInfo[0];
      await token721.safeMint(creatorWallet!.address, 10);
      const txnData = creatorWallet!.interface.encodeFunctionData("transferERC721", [
