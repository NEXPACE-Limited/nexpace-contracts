...
import { WarpMessage, IWarpMessenger } from "../WarpMessenger/IWarpMessenger.sol";
...
contract TeleporterMessenger is ITeleporterMessenger, ReentrancyGuards {
     ...
     */
    mapping(bytes32 sourceBlockchainID => ReceiptQueue.TeleporterMessageReceiptQueue receiptQueue) public receiptQueues;
     ...
     */
    function retrySendCrossChainMessage(TeleporterMessage calldata message) external senderNonReentrant {
        ...
        bytes32 messageID = calculateMessageID(blockchainID, message.destinationBlockchainID, message.messageNonce);
        ...
        require(existingMessageInfo.messageHash != bytes32(0), "TeleporterMessenger: message not found");
        ...
        emit SendCrossChainMessage(messageID, message.destinationBlockchainID, message, existingMessageInfo.feeInfo);
    ...
    function addFeeAmount(
        ...
        require(feeTokenAddress != address(0), "TeleporterMessenger: zero fee asset contract address");
        ...
        require(sentMessageInfo[messageID].messageHash != bytes32(0), "TeleporterMessenger: message not found");
        ...
        uint256 adjustedAmount = SafeERC20TransferFrom.safeTransferFrom(IERC20(feeTokenAddress), additionalFeeAmount);
    ...
    function receiveCrossChainMessage(uint32 messageIndex, address relayerRewardAddress) external receiverNonReentrant {
        ...
        require(warpMessage.originSenderAddress == address(this), "TeleporterMessenger: invalid origin sender address");
    ...
    function retryMessageExecution(
        ...
        // this contract, meaning that the message to be retried will not be found in any event.
        // Thus, don't need to initialize the blockchain ID here.
        bytes32 messageID = calculateMessageID(sourceBlockchainID, blockchainID, message.messageNonce);
        ...
        require(keccak256(abi.encode(message)) == failedMessageHash, "TeleporterMessenger: invalid message hash");
        ...
        require(message.destinationAddress.code.length > 0, "TeleporterMessenger: destination address has no code");
    ...
    function sendSpecifiedReceipts(
        ...
        TeleporterMessageReceipt[] memory receiptsToSend = new TeleporterMessageReceipt[](messageIDs.length);
        ...
        for (uint256 i; i < length; ++i) {
            ...
            require(
                receivedMessageID == calculateMessageID(sourceBlockchainID, blockchainID_, receivedMessageNonce),
    ...
    function calculateMessageID(
        ...
        return keccak256(abi.encode(address(this), sourceBlockchainID, destinationBlockchainID, nonce));
    }
    ...
    function _sendTeleporterMessage(
        ...
        bytes32 messageID = calculateMessageID(blockchainID_, messageInput.destinationBlockchainID, messageNonce_);
        ...
        emit SendCrossChainMessage(messageID, messageInput.destinationBlockchainID, teleporterMessage, adjustedFeeInfo);
    ...
    function _markReceipt(
        ...
        _relayerRewardAmounts[receipt.relayerRewardAddress][messageInfo.feeInfo.feeTokenAddress] += messageInfo
            .feeInfo
            .amount;

        emit ReceiptReceived(messageID, destinationBlockchainID_, receipt.relayerRewardAddress, messageInfo.feeInfo);
    }
